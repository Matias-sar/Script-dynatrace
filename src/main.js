  ...
  askForTypes((types) => {
    ...
    if (fs.existsSync(fileName)){
      ...
      filteredByTypes.forEach((service, index) =>{
        workSheet.cell(highestRow + index + 1, 1).string(service.displayName)
        workSheet.cell(highestRow + index + 1, 2).string(service.metadata.hostGroups)
        workSheet.cell(highestRow + index + 1, 3).string(service.softwareTechnologies[0].type)
        workSheet.cell(highestRow + index + 1, 4).string("TRUE")
      })
      ...
      var displayNameHeader = workSheet.cell(1, 1)
      displayNameHeader.string('DISPLAYNAME')
      displayNameHeader.style({font: {bold: true}})
    
      var hostGroupsHeader = workSheet.cell(1,2)
      hostGroupsHeader.string('HOSTGROUPS')
      hostGroupsHeader.style({font: {bold: true}})
    
      var typeHeader = workSheet.cell(1,3)
      typeHeader.string('TYPE')
      typeHeader.style({font: {bold: true}})
    
      var typeRestart = workSheet.cell(1,4)
      typeRestart.string('RESTART')
      typeRestart.style({font: {bold: true}})

      filteredByTypes.forEach((service, index) => {
        workSheet.cell(index + 2, 1).string(service.displayName);
        workSheet.cell(index + 2, 2).string(service.metadata.hostGroups);
        workSheet.cell(index + 2, 3).string(service.softwareTechnologies[0].type);
        workSheet.cell(index + 2, 4).string("TRUE");
      });
